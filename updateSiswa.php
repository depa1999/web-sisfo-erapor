...
if (isset($_POST['cari'])){
	$cari=$_POST['cari'];
	$sql="select * from siswa where nisn like'%$cari%' or
						  nis like '%$cari%' or
...
if (isset($_POST['cari'])){
	...
	$sql="select * from siswa order by kelas asc limit $awalData,$jmlDataPerHal";		
}
...
<div class="utama">
	...
	<div class="text-center"><a href="prnSiswaAllPdf.php"><span class="fas fa-print">&nbsp;Print</span></a></div>
	...
	if ($kosong){
		...
		while($row=mysqli_fetch_assoc($hasil)){
			...
			<tr>
				...
				<td><?php echo $row["tempat_siswa"]?></td>
				<td><?php echo $row["tgl_siswa"]?></td>
				<td><?php echo $row["jenkel_siswa"]?></td>
				<!--<td><img src="<?php echo "foto/".$row["foto"]?>" height="50"></td>-->
				<td>
				<a class="btn btn-outline-primary btn-sm" href="editSiswa.php?kode=<?php echo $row['nis']?>">Koreksi</a>
				<a class="btn btn-outline-danger btn-sm" href="hpsSiswa.php?kode=<?php echo $row["nis"]?>" id="linkHps" onclick="return confirm('Yakin dihapus nih?')">Hapus</a>
				</td>
			</tr>
			<?php 
			$no++;
